...
var pkg4Jsn = {
		<c:forEach var="i" items="${dvcList}" varStatus="idx">
			unit${idx.index+1} : {name: '${i.dvc_nm}',dvc_id : '${i.dvc_id}'},
		</c:forEach>
}
...
function fnTable(){
	ClAjax.fnCallAjax({
		url : '/pmng/op_cylinderCylceAjax.do',
		errHandle : true,
		sucFn: function(data){
			var list = data.data.list;
			var list_str;
...
// 						console.log(item.var_id);
		...
		compltFn : function() {
			timeout = setTimeout(function(){
				fngetPVColorList();
				//console.log('PV 색상 인터발 테스트');
	        }, intvl);
		},
	}); 
}

// 이미지에 맞게 CTQ Mean & NG Rate 테이블 이동 
function changeCtqTableCss(){
	(c_type == 'A') ? $(".ctq_tbl_2").css('top','129px') : $(".ctq_tbl_2").css('top','137px');
}
</script>